...
def encode(v, level = 1, enable_max = False, **kwargs):
    if enable_max:
    ...
    for interval_l in range(level):  #range should be [interval_l/level, (interval_l + 1)/level]
        ...
        interval_mask = w.clone().byte().type(torch.bool)
        if w.is_cuda:
        ...
        mask = torch.distributions.Bernoulli(probs).sample().byte().type(torch.bool)
        if w.is_cuda:
            mask = mask.cuda()
